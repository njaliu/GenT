...
FAKE_SYS_LIB = '/home/aliu/Research/Projects/pycparser/utils/fake_libc_include'
CSMITH_RUNTIME = "/home/aliu/Research/Projects/csmith-2.2.0/runtime"
CSMITH_MINIMAL_MACRO = 'CSMITH_MINIMAL'
...
# def generate_line(code, origin_file, gen_path):
...
def preprocess(origin_file, preprocess_path):
	...
	subprocess.call(PP_CMD + origin_file + " -I " + CSMITH_RUNTIME + " > " + filename_pp, shell=True)
	...
	for root, dirs, files in os.walk(origin_path):
		for f in files:
			if f.endswith(".c") or f.endswith(".i"):
				num += 1
				#try:
				generate(f, origin_path, gen_path)
				'''
...
if __name__ == "__main__":
	if len(sys.argv) == 4:
		...
		print("\nUsage: \n    prog_monkey_csmith.py origin_dir preprocess_dir generate_dir\n")
